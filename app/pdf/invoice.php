<?php
	...
	if($datos_venta->rowCount()==1){
		...
		$pdf->AddPage();
		$pdf->Image(APP_URL.'app/views/img/logo1.png',145,8,55,55,'PNG');

		$pdf->SetFont('Arial','B',16);
		$pdf->SetTextColor(191,7,27);
		...
		$pdf->Cell(150,9,iconv("UTF-8", "ISO-8859-1","Claudia Pelaez"),0,0,'L');

		$pdf->Ln(5);

		$pdf->Cell(150,9,iconv("UTF-8", "ISO-8859-1","NIT: 52026759-3"),0,0,'L');

		$pdf->Ln(5);

		$pdf->Cell(150,9,iconv("UTF-8", "ISO-8859-1","Regimen Simlificado  No responsable de IVA"),0,0,'L');
		...
		$pdf->SetFillColor(191,7,27);
		$pdf->SetDrawColor(191,7,27);
		...
		$pdf->Cell(34,7,iconv("UTF-8", "ISO-8859-1",MONEDA_SIMBOLO.number_format($datos_venta['venta_descuento'],MONEDA_DECIMALES,MONEDA_SEPARADOR_DECIMAL,MONEDA_SEPARADOR_MILLAR).' '.MONEDA_NOMBRE),'',0,'C');
        ...
        $pdf->SetFont('Arial','',10);
        $pdf->MultiCell(0,5,iconv("UTF-8", "ISO-8859-1","** Que Jehová te bendiga y sea un escudo a tu alrededor **"),0,'C',false);

        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(0,7,iconv("UTF-8", "ISO-8859-1","SALMO 5-12"),'',0,'C');
