<?php
	...
	class saleController extends mainModel{
        ...
        public function ListarProdRapidosControlador(){

            /*== Seleccionando productos en la DB ==*/
            $datos_productos=$this->ejecutarConsulta("SELECT * FROM producto LIMIT 6");

            if($datos_productos->rowCount()>=1){

				$datos_producto=$datos_productos->fetchAll();

				$result='';

				foreach($datos_producto as $rows){
					$result.='
					<div class="reg-4 pd-3 card mg-2 cursor dp-flex jfy-ctn-center alg-itm-center" onclick="agregar_codigo(\''.$rows['producto_codigo'].'\')">
						<p style="font-size: larger;"> <b>'.$rows['producto_nombre'].'</b> Talla <i>'.$rows['producto_talla'].'</i></p>
					</div>
                    ';
				}
				return $result;
        ...
        public function agregarProductoCarritoControlador(){
            ...
            if(empty($_SESSION['datos_producto_venta'][$codigo])){
                ...
                $_SESSION['datos_producto_venta'][$codigo]=[
                    ...
                    "venta_detalle_descripcion"=>$campos['producto_nombre'].' Talla '. $campos['producto_talla']
                ];

                $_SESSION['alerta_producto_agregado']="Se agrego <strong>".$campos['producto_nombre'].' Talla '. $campos['producto_talla']."</strong> a la venta";
            }else{
                ...
                $_SESSION['alerta_producto_agregado']="Se agrego +1 <strong>".$campos['producto_nombre'].' Talla '. $campos['producto_talla']."</strong> a la venta. Total en carrito: <strong>$detalle_cantidad</strong>";
            }
        ...
        public function actualizarProductoCarritoControlador(){
            ...
            if($precio==""){
            	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No podemos actualizar el precio de producto debido a que faltan algunos parámetros de configuración",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            /*== comprobando precio de productos ==*/
            if($precio<=0){
                $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"Debes de introducir un precio mayor a 0",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            if(!empty($_SESSION['datos_producto_venta'][$codigo])){
			    ...
			    //     exit();
                // }
				$diferencia_productos='No se modificó la cantidad del producto';
                ...
                }elseif($cantidad<$_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"]){
                    $diferencia_productos="Quito -".($_SESSION['datos_producto_venta'][$codigo]["venta_detalle_cantidad"]-$cantidad);
                }
				if($precio<>$_SESSION['datos_producto_venta'][$codigo]["venta_detalle_precio_venta"]){
                    $diferencia_productos.=" - modifico a ".$precio." el precio del producto";
                }
                ...
                $detalle_total=$detalle_cantidad*$precio;
                $detalle_total=number_format($detalle_total,MONEDA_DECIMALES,'.','');
                ...
                $_SESSION['datos_producto_venta'][$codigo]=[
					...
					"producto_stock_total_old"=>$campos['producto_stock_total'],
                    "venta_detalle_precio_compra"=>$campos['producto_precio_compra'],
                    "venta_detalle_precio_venta"=>$precio,
                    "venta_detalle_cantidad"=>$detalle_cantidad,
                    "venta_detalle_total"=>$detalle_total,
                    "venta_detalle_descripcion"=>$campos['producto_nombre'].' Talla '.$campos['producto_talla']
                ];

                $_SESSION['alerta_producto_agregado']="$diferencia_productos <strong>".$campos['producto_nombre'].' Talla '.$campos['producto_talla']."</strong> a la venta. Total en carrito <strong>$detalle_cantidad</strong>";
        ...
        public function registrarVentaControlador(){
            ...
            $venta_descuento=$this->limpiarCadena($_POST['venta_descuento']);
			...
			if($this->verificarDatos("[0-9.]{1,25}",$venta_descuento)){
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"El descuento de la venta no coincide con el formato solicitado",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
            ...
            $venta_descuento=number_format($venta_descuento,MONEDA_DECIMALES,'.','');
            ...
            $venta_cambio=($venta_pagado+$venta_descuento)-$venta_total_final;
            $venta_cambio=number_format($venta_cambio,MONEDA_DECIMALES,'.','');


            /*== Calculando total en caja ==*/
            $movimiento_cantidad=($venta_pagado-$venta_descuento)-$venta_cambio;
			...
			$datos_venta_reg=[
				...
				[
					"campo_nombre"=>"venta_descuento",
					"campo_marcador"=>":Descuento",
					"campo_valor"=>$venta_descuento
				],
				[
		...
		public function eliminarVentaControlador(){
		    ...
		    if($check_detalle_venta>0){
				...
				$productos_detalle_venta = mysqli_query($conexion,"SELECT * FROM venta_detalle WHERE venta_codigo='".$datos['venta_codigo']."'");
				
				while ($producto = mysqli_fetch_array($productos_detalle_venta)) {
					
					$productos_actuales = mysqli_query($conexion,"SELECT * FROM producto WHERE producto_id='".$producto['producto_id']."'");

					while ($productos_act = mysqli_fetch_array($productos_actuales)) {
						$cantidad_nueva = $producto['venta_detalle_cantidad']+$productos_act['producto_stock_total'];
						$id_producto = $producto['producto_id'];

						$update_cantidad_productos = mysqli_query($conexion,"UPDATE producto SET producto_stock_total = $cantidad_nueva WHERE producto_id = $id_producto");
						
						if (!$update_cantidad_productos) {
							$alerta=[
								"tipo"=>"simple",
								"titulo"=>"Ocurrió un error inesperado",
								"texto"=>"No hemos podido eliminar la venta del sistema, no se devolvieron las cantidades. Por favor intente nuevamente",
								"icono"=>"error"
							];
							return json_encode($alerta);
							exit();
