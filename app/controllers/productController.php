<?php
	...
	class productController extends mainModel{
		...
		public function registrarProductoControlador(){
            ...
            if($codigo=="" || $nombre=="" || $talla=="" || $precio_compra=="" || $precio_venta=="" || $stock==""){
            	$alerta=[
					"tipo"=>"simple",
		    ...
		    if($this->verificarDatos("[0-9.]{1,25}",$talla)){
		    	$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"LA TALLA no coincide con el formato solicitado",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		    ...
		    # Comprobando la talla de producto #
		    $check_talla=$this->ejecutarConsulta("SELECT producto_nombre FROM producto WHERE producto_codigo='$codigo' AND producto_nombre='$nombre' AND producto_talla='$talla'");
		    if($check_talla->rowCount()>=1){
		        $alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"Ya existe un producto registrado con el mismo nombre, talla y código de barras",
					"icono"=>"error"
				];
				return json_encode($alerta);
		        exit();
		...
		public function listarProductoControladorTable(){

			// $tipo=$this->limpiarCadena($tipo);
			// $rolUsuario=$this->limpiarCadena($rolUsuario);
			...
			$consulta_datos="SELECT $campos FROM producto INNER JOIN categoria ON producto.categoria_id=categoria.categoria_id ORDER BY producto_nombre ASC";
			...
			foreach($datos as $rows){
				...
				$tabla.='</p>
					...
					<td><p><strong>'.$rows['producto_nombre'].'</strong></p></td>
					<td><p><strong>CODIGO:</strong> '.$rows['producto_codigo'].'</p></td> 
					<td><p><strong>PRECIO:</strong> $'.$rows['producto_precio_venta'].'</p></td>
					<td><p><strong>STOCK:</strong> '.$rows['producto_stock_total'].'</p></td>
					<td><p><strong>CATEGORIA:</strong> '.$rows['categoria_nombre'].'</p></td>
					<td>
		...
		public function actualizarProductoControlador(){
		    ...
		    # Comprobando talla de producto #
		    if($datos['producto_nombre']!=$nombre){
			    $check_nombre=$this->ejecutarConsulta("SELECT producto_nombre FROM producto WHERE producto_codigo='$codigo' AND producto_nombre='$nombre' AND producto_talla='$talla'");
			    if($check_nombre->rowCount()>=1){
			        $alerta=[
						"tipo"=>"simple",
						"titulo"=>"Ocurrió un error inesperado",
			...
			if($this->actualizarDatos("producto",$producto_datos_up,$condicion)){
				$alerta=[
					...
					"texto"=>"Los datos del producto '".$datos['producto_nombre'].' '. $datos['producto_talla']."' se actualizaron correctamente",
					"icono"=>"success"
				];
			}else{
				$alerta=[
					"tipo"=>"simple",
					"titulo"=>"Ocurrió un error inesperado",
					"texto"=>"No hemos podido actualizar los datos del producto '".$datos['producto_nombre'].' '. $datos['producto_talla']."', por favor intente nuevamente",
					"icono"=>"error"
				];
			}
